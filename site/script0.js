...
async function renderInbox(){
  ...
  try{
    ...
    if(!filtered.length){
      $("inboxBody").innerHTML=
        `<tr><td colspan="6" class="empty">No Gmail messages found.</td></tr>`;
      return;
    }

    $("inboxBody").innerHTML=filtered.map(email=>`
      <tr class="click"
          onclick='openGmailEmail(${JSON.stringify(email)})'>

        <td>${esc(email.from||"")}</td>

        <td>${esc(email.subject||"(No subject)")}</td>

        <td>
          <span class="badge obs">GMAIL</span>
        </td>
        ...
        <td>
          <button class="btn"
            onclick='event.stopPropagation();openGmailEmail(${JSON.stringify(email)})'>
            Open
          </button>
        </td>
...
function openGmailEmail(email){
  ...
  const existing=window.openGmailEmailView;
  if(existing){
    existing(email);
    return;
  }
